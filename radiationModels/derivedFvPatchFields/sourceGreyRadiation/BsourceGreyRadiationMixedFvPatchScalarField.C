...
#include "AsourceGreyRadiationMixedFvPatchScalarField.H"
...
{									
	...
	scalarField& Qr = const_cast<fvPatchField<double>&>(quad.currQrLambda(lambdaId).boundaryField()[patchI]);
	Qr = Iw*nAve;
	
	
	scalarField& Qin = const_cast<fvPatchField<double>&>(quad.currQinLambda(lambdaId).boundaryField()[patchI]);
